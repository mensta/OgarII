const { intersects, fullyIntersects, getQuadIntersect, getQuadFullIntersect } = require("./Constants");

/**
 * @template T
 * @typedef {{ __root?: QuadTree<T>, range: Range }} QuadItem
 * @typedef {-1 | 0 | 1 | 2 | 3} Quad
*/
...
class QuadTree {
    ...
    containsAny(range, selector) {
        for (let i = 0, l = this.items.length, item; i < l; i++)
            if (intersects(range, (item = this.items[i]).range) && (!(selector instanceof Function) || selector(item)))
                return true;
